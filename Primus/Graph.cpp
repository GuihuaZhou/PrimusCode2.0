...
// add by guihuazhou
Graph::Graph(int level, int pos, int SpineNodes, int ToRNodes, int LeafNodes, int nPods)
{
	build_graph(level, pos, SpineNodes, ToRNodes, LeafNodes, nPods);
}

void 
Graph::remove_edge_(int start_vertex, int end_vertex)
{
	BaseVertex* start_vertex_pt = get_vertex(start_vertex);
	BaseVertex* end_vertex_pt = get_vertex(end_vertex);
	m_mpEdgeCodeWeight.erase(get_edge_code(start_vertex_pt, end_vertex_pt));
	get_vertex_set_pt(end_vertex_pt, m_mpFaninVertices)->erase(start_vertex_pt);
	get_vertex_set_pt(start_vertex_pt, m_mpFanoutVertices)->erase(end_vertex_pt);
}

void 
Graph::build_edge(int start_vertex, int end_vertex)
{
...
// 节点分配规则，即直接去掉小数点
void 
Graph::build_graph(int level, int pos, int SpineNodes, int ToRNodes, int LeafNodes, int nPods)
{
	m_nVertexNum = SpineNodes + (ToRNodes + LeafNodes) * nPods;
	int m_Pod = pos / ToRNodes;
	for (int i = 0; i < LeafNodes; i++)// src -- leaf
	{
		build_edge(10000 + pos, 2000 + (m_Pod * LeafNodes) + i);
	}
	for (int i = 0; i < LeafNodes; i++)// leaf -- spine
	{
		for (int j = 0; j< SpineNodes / LeafNodes; j++)
		{
			build_edge(2000 + (m_Pod * LeafNodes) + i, 30 + i * (SpineNodes / LeafNodes) + j);
		}
	}
	for (int i = 0; i < SpineNodes; i++)// spine -- leaf
	{
		for (int j = 0; j < nPods; j++)
		{
			if (j == m_Pod) continue;
			build_edge(30 + i, 2000 + (i / (SpineNodes / LeafNodes)) + j * LeafNodes);
		}
	}
	for (int i = 0; i < LeafNodes * nPods; i++)
	{
		for (int j = 0; j < ToRNodes; j++)
		{
			if ((i / LeafNodes * ToRNodes + j) == pos) continue;
			build_edge(2000 + i, 10000 + i / LeafNodes * ToRNodes + j);
		}
	}
