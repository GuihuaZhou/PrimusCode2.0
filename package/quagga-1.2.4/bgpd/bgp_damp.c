...
bgp_reuse_timer (struct thread *t)
{
  ...
  fprintf(fp, "bgp_reuse_timer,thread name is %s,system time is %f\n",t->funcname,tv.tv_sec+tv.tv_nsec*0.000000001);
  fflush(fp);
  fclose(fp);
  //end
		   ...
		   afi_t afi, safi_t safi, int attr_change)
{
  ...
  fseek(fp,0L,SEEK_END);
  fprintf(fp, "bgp_damp_withdraw,prefix is %s,system time is %f\n",inet_ntoa(rn->p.u.prefix4),tv.tv_sec+tv.tv_nsec*0.000000001);
  fflush(fp);
  fclose(fp);
  //end
