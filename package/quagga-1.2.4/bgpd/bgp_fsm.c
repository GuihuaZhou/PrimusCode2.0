...
bgp_timer_set (struct peer *peer)
{
  ...
  struct timespec tv;
  FILE * fp;
  if ((fp=fopen("/home/guolab/output/statusRecord.txt","a+"))==NULL)
  {
    printf("Cannot open statusRecord.txt.txt\n");
    exit(0);
  }
  // end
  ...
  switch (peer->status)
    ...
    fprintf(fp,"\nIdle,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
    fflush(fp);
    // end
    ...
    fseek(fp,0L,SEEK_END);
    fprintf(fp,"Connect,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
    fflush(fp);
    // end
      BGP_TIMER_OFF (peer->t_start);
    ...
    fseek(fp,0L,SEEK_END);
    fprintf(fp,"Active,I am passive,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
    fflush(fp);
    // end
	}
      else
    ...
    fseek(fp,0L,SEEK_END);
    fprintf(fp,"Active,I am initiative,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
    fflush(fp);
    // end
	}
      BGP_TIMER_OFF (peer->t_holdtime);
  ...
  //add by me
   clock_gettime(CLOCK_MONOTONIC,&tv);
    ...
    fseek(fp,0L,SEEK_END);
    fprintf(fp,"Established,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
    fflush(fp);
    // end
    ...
    case Deleted:
      ...
      // fseek(fp,0L,SEEK_END);
      // fprintf(fp,"\nDeleted,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
      // fflush(fp);
      // // end
    ...
    case Clearing:
      ...
      fseek(fp,0L,SEEK_END);
      fprintf(fp,"\nClearing,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
      fflush(fp);
      // end
    }
}
...
bgp_start (struct peer *peer)
{
  ...
  switch (status)
	...
	plog_debug (peer->log, "%s [FSM] Connect error", peer->host);
      ...
      fseek(fp,0L,SEEK_END);
      fprintf(fp,"connect_error,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
      fflush(fp);
      // end
      break;
	...
	plog_debug (peer->log, "%s [FSM] Connect immediately success",
      ...
      fseek(fp,0L,SEEK_END);
      fprintf(fp,"connect_success,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
      fflush(fp);
      // end
      break;
      ...
      if (peer->fd < 0)
	{
      ...
      fseek(fp,0L,SEEK_END);
      fprintf(fp,"connect_in_progress,peer is %s,system time is %.9f\n",inet_ntoa(peer->remote_id),tv.tv_sec+tv.tv_nsec*0.000000001);
      fflush(fp);
      // end
      break;
    }
...
bgp_establish (struct peer *peer)
{
  ...
  // modify by me
  BGP_TIMER_ON (peer->t_routeadv, bgp_routeadv_timer, 0);//建立peer后立即发送update
  // BGP_TIMER_ON (peer->t_routeadv, bgp_routeadv_timer, 1);// normal
