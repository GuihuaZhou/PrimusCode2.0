...
bgp_process_main (struct work_queue *wq, void *data)
{
  ...
  // {
  //   printf("Cannot open updateKernelTimeRecord.txt.txt\n");
  //   exit(0);
  // }

  // fseek(fp,0L,SEEK_END);
  // fprintf(fp, "bgp_process_main,system time is %.9f\n",tv.tv_sec+tv.tv_nsec*0.000000001);
  // fflush(fp);
  // fclose(fp);
  // // end
            ...
            struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  FILE * fp;
  if ((fp=fopen("/home/guolab/output/updateBGPRIBTimeRecord.txt","a+"))==NULL)
  {
    printf("Cannot open updateBGPRIBTimeRecord.txt.txt\n");
    exit(0);
  }

  fseek(fp,0L,SEEK_END);
  fprintf(fp, "bgp_update,prefix is %s,system time is %f\n",inet_ntoa(p->u.prefix4),tv.tv_sec+tv.tv_nsec*0.000000001);
  fflush(fp);
  fclose(fp);
  // end
	     ...
	     struct prefix_rd *prd, u_char *tag)
{
  ...
  fseek(fp,0L,SEEK_END);
  fprintf(fp, "bgp_withdraw,perfix is %s,system time is %f\n",inet_ntoa(p->u.prefix4),tv.tv_sec+tv.tv_nsec*0.000000001);
  fflush(fp);
  fclose(fp);
  // end
