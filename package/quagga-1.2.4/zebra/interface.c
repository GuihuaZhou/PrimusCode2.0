...
if_up (struct interface *ifp)
{
  ...
  fprintf(fp, "\nif_up,if index is %d,system time is %.9f\n\n",ifp->ifindex,tv.tv_sec+tv.tv_nsec*0.000000001);
  fflush(fp);
  fclose(fp);
  // end
  ...
  // add by me
  FILE * fpA;
  if ((fpA=fopen("/home/guolab/output/updateKernelTimeRecord.txt","a+"))==NULL)
  {
...
if_down (struct interface *ifp)
{
  ...
  // fseek(fp,0L,SEEK_END);
  // fprintf(fp, "\nif_down,if index is %d,system time is %.9f\n\n",ifp->ifindex,tv.tv_sec+tv.tv_nsec*0.000000001);
  // fflush(fp);
  // fclose(fp);
  // // end
